...
async def get_volunteer_impact_metrics(volunteer_id: UUID, user: UserContext = Depends(require_any)):
    """
    Get impact metrics for a specific volunteer.
    NGO can see any; Volunteer can only see their own.
    """
    if user.role == "volunteer" and str(volunteer_id) != user.user_id:
         raise HTTPException(status_code=403, detail="Forbidden: You can only see your own metrics")
...
async def get_district_impact_metrics(district_name: str, user: UserContext = Depends(require_any)):
    """Get impact metrics for a specific district. Authenticated only."""
    try:
        response = (
...
async def get_all_district_metrics(user: UserContext = Depends(require_any)):
    """Get impact metrics for all districts. Authenticated only."""
    try:
...
async def create_task_template(template: TaskTemplate, user: UserContext = Depends(require_ngo)):
    """Create a new task template for recurring tasks. NGO only."""
    try:
        payload = {
            "name": template.name,
...
async def get_task_templates(active_only: bool = True, user: UserContext = Depends(require_ngo)):
    """Get all task templates for the current NGO. NGO only."""
    try:
        query = supabase.table("task_templates").select("*").eq("ngo_id", user.ngo_id)
...
async def create_task_from_template(template_id: UUID, district: str, ward: str, lat: float, lng: float, user: UserContext = Depends(require_ngo)):
    """Create a new task from a template. NGO only."""
    try:
        # Get template
        template_response = (
        ...
        if not template_response.data:
            raise HTTPException(status_code=404, detail="Task template not found in your NGO.")
